...
async fn a_proposal_published_on_one_node_appears_in_the_indexers_snapshot() {
    ...
    let mut gossip = openfiat_gossip::GossipService::new(producer_node, event_store, producer_keypair, vec![], Subscription::All);
    ...
    tokio::time::timeout(Duration::from_secs(15), drive_and_wait).await.expect("producer never connected to the indexer");

    let proposal_id = service.create_proposal("ofp-1", "Increase Reservation Timeout", "Raise the validation window.", ProposalCategory::Protocol).unwrap();

    let wait_for_snapshot = async {
        loop {
            service.drive_once().await;
            if snapshot.read().unwrap().proposals.iter().any(|p| p.id == proposal_id.as_str()) {
                break;
            }
        }
    };
    tokio::time::timeout(Duration::from_secs(15), wait_for_snapshot).await.expect("the indexer never reflected the published proposal");

    let view = snapshot.read().unwrap();
    let indexed = view.proposals.iter().find(|p| p.id == proposal_id.as_str()).unwrap();
